...
- 支持 14 种翻译引擎（OpenAI、DeepSeek、Claude、Gemini、DeepL、Google、百度、本地 NLLB 等）
- 有图形界面，也有命令行
- 有翻译缓存、术语表、人工校对
- 有整合包发布者需要的“一键安装到 resourcepacks”

---

## 目录

- [第一部分：普通用户教程（面向所有人）](#第一部分普通用户教程面向所有人)
  - [第 1 步：下载程序](#第-1-步下载程序)
  - [第 2 步：第一次打开](#第-2-步第一次打开)
  - [第 3 步：申请一个翻译引擎的 API Key](#第-3-步申请一个翻译引擎的-api-key)
  - [第 4 步：把 API Key 填进程序](#第-4-步把-api-key-填进程序)
  - [第 5 步：选择你的整合包](#第-5-步选择你的整合包)
  - [第 6 步：开始翻译](#第-6-步开始翻译)
  - [第 7 步：把翻译装进游戏](#第-7-步把翻译装进游戏)
  - [第 8 步（可选）：人工校对](#第-8-步可选人工校对)
...
> **不懂电脑也没关系。** 下面每一步都写清楚了“点哪里、填什么、看到什么算成功”。
...
3. 把下载好的 `MCModTranslator.exe` **移动到一个你记得住的地方**，例如：
...
> 第一次打开时，先别急着点“开始翻译”，我们还需要配置一个“翻译引擎”。

---

## 第 3 步：申请一个翻译引擎的 API Key

“翻译引擎”就是真正帮你把英文变中文的那个服务。程序自己不会翻译，它只是调用别人的服务。

**推荐给新手的两个选择：**

### 选择 A：DeepSeek（便宜、中文好、注册简单）

1. 打开 <https://platform.deepseek.com/>
2. 注册一个账号（手机号或邮箱）
3. 登录后，左侧菜单找到 **API Keys** → **创建 API Key**
4. 复制出来的那一长串字符（类似 `sk-xxxxxxxxxxxxxxxx`）
5. **立刻粘贴到记事本里保存**，这个页面关掉后就再也看不到完整 Key 了
6. 在左侧菜单找到 **充值**，充 10 元人民币（1 元就能翻译几千条，10 元足够用很久）

### 选择 B：OpenAI（效果最好、价格略高）

1. 打开 <https://platform.openai.com/>
2. 注册并登录
3. 右上角头像 → **View API keys** → **Create new secret key**
4. 复制出来，同样马上存到记事本
5. 需要绑定信用卡并充值（最低 5 美元）

> 💰 **成本参考**：一个 100 个 mod 的整合包，用 DeepSeek 大约花 **0.5~2 元**。

> 🔒 你的 API Key 只会加密保存在你自己的电脑上（`~/.mc_mod_translator/secrets.enc`），不会上传到任何地方。

---

## 第 4 步：把 API Key 填进程序

回到程序主界面。

1. 找到 **翻译引擎** 区块。
2. 在下拉框里选你刚才注册的那家：
   - 用 DeepSeek 的 Key → 选 `deepseek`
   - 用 OpenAI 的 Key → 选 `openai`
3. 点击 **配置引擎...** 按钮，弹出配置窗口。

![引擎配置](docs/images/03-engine-dialog.png)

4. 在弹出的窗口里，找到 **API Key** 那一栏，粘贴你刚才保存的 Key。
5. 点击 **测试连接**，等几秒：
   - ✅ 弹出“成功。示例返回：…” → 没问题，继续
   - ❌ 弹出“失败” → 检查：Key 有没有多复制空格？余额够不够？网络是否正常？
6. 点击 **保存** 关闭配置窗口。

> ⚠️ 关闭配置窗口后，**还要在主窗口点一次“保存配置”**，否则下次打开程序会丢失。

> 🔒 `region`（区域）这类字段不是密码，会明文显示，不用担心。
...
| --- | --- |
| 目标语言 | `简体中文 (zh_cn)` |
| 输出模式 | `合并为一个资源包`（首次使用推荐） |
| 自动安装到 resourcepacks | ✅ 勾上（省事） |
| 保留已有 zh_cn 翻译 | ✅ 勾上（避免重译） |
| 并发数 | `8`（默认即可，网络差改成 `3`） |
...
下面的日志会实时显示每条翻译的情况，包括重试、错误、缓存命中。

> ⏱ **时间参考**：100 个 mod、大约 2 万条文本，用 DeepSeek 大约 10~25 分钟。

### 如果想中途停止

点 **取消翻译** 按钮。程序会在完成当前正在翻译的几条后停下，已经翻译好的内容会保存到缓存里，下次继续翻译时不会重复花钱。
...
- 检查电脑能不能上网
- 用 DeepSeek / OpenAI 需要能访问对应网站
- 如果用了代理，试试关闭或开启代理

### Q6：提示“429 限流”

免费额度或并发太高了。程序会自动等待重试。也可以把主界面 **并发数** 改成 `3` 或 `2`。

### Q7：提示“余额不足”

去对应平台充值。DeepSeek 最低 1 元，OpenAI 最低 5 美元。

### Q8：API Key 保存在哪里？安全吗？

- 保存在 `C:\Users\<你的用户名>\.mc_mod_translator\secrets.enc`
- 使用 Fernet 加密，密钥在 `key.bin`，权限 0600（只有你自己能读）
- **不会**上传到任何服务器
...
- 该 mod 的语言文件 key 是动态生成的，无法预先翻译

### Q11：翻译很慢

- 把 **并发数** 调大（网络好可以到 16~32）
- 换个更快的引擎（DeepSeek 比 OpenAI 快）
- 第一次翻译本来就慢，第二次因为有缓存会快很多

### Q12：翻译质量不好

- 换更强的模型：OpenAI 配置里把 `model` 改成 `gpt-4o`，DeepSeek 改成 `deepseek-reasoner`
- 使用人工校对（见第 8 步）
- 编辑 `~/.mc_mod_translator/glossary.csv`，加入你自己的术语表（格式：`英文,中文`）

### Q13：某些词应该统一翻译，但每次都不一样
...
```bash
# 使用整合包根目录，自动安装
mcmt translate --pack-root ./MyPack --engine deepseek --auto-install

# 指定 mods 文件夹和输出目录
mcmt translate --mods-dir ./mods --output-dir ./out --engine openai
...
# 测试引擎连接
mcmt engines test --engine openai
...
1. 更新 `pyproject.toml` 与 `mc_mod_translator/__init__.py` 里的版本号
2. 更新 `CHANGELOG.md`
3. `git tag v0.2.0 && git push --tags`
4. 在 GitHub Releases 页面：
   - 标题：`v0.2.0`
   - 说明：粘贴 CHANGELOG 对应段
   - 上传 `MCModTranslator.exe`
5. 发布
